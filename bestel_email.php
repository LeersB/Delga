...
<div style="background-color:#fff;margin:30px;box-sizing:border-box;font-size:16px;">
        ...
        <table style="border-collapse:collapse;width:100%;">
            ...
            <?php foreach ($producten_winkelmand as $product): ?>
                <tr>
                    ...
                    <?php if ($product['optie_eenheidsprijs'] > 0): ?>
                        <td style="text-align:right;">€ <?= number_format($product['optie_eenheidsprijs'] * $product['aantal'], 2) ?></td>
                    <?php else: ?>
                        <td style="text-align:right;">€ <?= number_format($product['meta']['eenheidsprijs'] * $product['aantal'], 2) ?></td>
                    <?php endif; ?>
                </tr>
            <?php endforeach; ?>
            </tbody>
    ...
    <p style="padding:20px 40px 60px 40px;margin:0;box-sizing:border-box;font-size:16px;">
        ...
        <?=$adres_straat?>&nbsp;<?=$adres_nr?><br>
        <?=$adres_postcode?>&nbsp;<?=$adres_plaats?>
    </p>

<h1 style="padding:40px;box-sizing:border-box;font-size:24px;color:#394453;background-color:#EBFFD7;margin:0;" align="right">test.delga.be &copy; 2021</h1>
</div>
</body>
</html>
